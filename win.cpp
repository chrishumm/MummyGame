...
win::win(int m_bars, int m_mummy, int m_lives) : GameConditions(m_bars,m_mummy,m_lives)
{
	captured = m_mummy;
	lives = m_lives;
	bar = m_bars;
...
void win::message() //takes inherited variables
{
	system("CLS"); //clears the screen
	cout <<"Congratulations! You have won!\nYou captured: " << captured <<" mummies!. You still have: "<< lives << " lives left! \n\nDo you want to play again?\n1) Yes \n2) Exit\n3) Save Hiscore";
	decision = NULL; //Above outputs congratulations message and I set char to null
	loop = 0; // loop is set to 0 to start our while loop
	while(loop == 0)
	{
	decision = _getch(); //we store decision from getch
	switch(decision)//start a switch statement based on that imput
	{
	case '1': //if player chose new game
		loop++;//break loop
		level::turn = 1;			//We reset the static variables
		level::m_no_of_spades = 100;//as these are defined once in the program
		level::caughtmummies = 0;	//
			for(int i = 0;i < 100;i++)		//
			{								//As trapped mummies is also static
				if(TrappedMummies[i] == 1)	//I need to reset each array member to 0
					TrappedMummies[i] = 0;	//
				else{}
			}
...
void win::saveHiscore(int f) //After taking in the score variable
{
